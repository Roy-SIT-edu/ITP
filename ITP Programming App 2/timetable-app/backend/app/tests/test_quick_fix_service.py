...
def test_quick_fix_suggests_clean_room_change_for_room_conflict(db_session):
    ...
    availability = QuickFixService().availability(db_session, run.id)

    assert availability["by_session_id"][str(sessions[0].id)] is True
    assert availability["by_conflict_id"][str(violation.id)] is True
